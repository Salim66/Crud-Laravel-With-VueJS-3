...
const newProduct = () => {
    return router.push('/product/new');
}

const getProducts = async () => {
    const response = await axios.get('api/get_all_product');
    ...
    <div class="container">
        <div class="products__list table  my-3">
              ...
              <div class="table--items products__list__item" v-for="item in products" :key="item.id" v-if="(products.length > 0)" >
                  ...
                  <div>
                      <button class="btn-icon btn-icon-success" @click="productEdit(item.id)" >
                          <i class="fa fa-pencil"></i>
                      </button>&nbsp;
                      <button class="btn-icon btn-icon-danger" @click="productDelete(item.id)" >
